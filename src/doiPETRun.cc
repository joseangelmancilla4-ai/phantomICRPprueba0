...
// ***** AÑADÍ ESTO AQUÍ *****
        G4double truthEnergyPrincipal = 0.;    // Energía truth del hit principal
       G4ThreeVector globalPositionPrincipal; // Posición global del hit principal
// ***************************
...
// ***** AÑADÍ ESTO AQUÍ *****
                                truthEnergyPrincipal = (*mitr).second->GetTruthEnergy();
                                globalPositionPrincipal = (*mitr).second->GetGlobalPosition();
// ***************************
		...
		// ***** MODIFICA ESTA LÍNEA *****
		doiPETAnalysis::GetInstance()->ReadOut(blockID, crystalID, interactionTime, timeOfAnnihil, interactionPos, totalEdep, truthEnergyPrincipal, globalPositionPrincipal);
