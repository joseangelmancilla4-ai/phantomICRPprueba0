...
//doiPETPrimaryGeneratorAction.hh
...
/// Primary generator: soporta fuentes por órgano con isótopo y modo (decay|direct)
...
public:
    ...
    // Comando invocado por messenger:
    // addOrganSource <organID> <isotope> <intensity> [mode]
    void AddOrganSource(G4int organID, const G4String& isotope, G4double intensity, const G4String& mode = "decay");

private:
    void DefineCommands();
    void SampleDirectPositronEnergy(G4double Emax_keV, G4double &kineticEnergy_keV);
    ...
    struct SourceSpec {
        std::string isotope; // "F18", "C11", "N13"
        double intensity;    // relative weight
        std::string mode;    // "decay" or "direct"
    };

    // map organID -> SourceSpec
